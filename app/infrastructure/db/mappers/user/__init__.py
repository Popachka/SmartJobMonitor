...
def user_to_model(user: User) -> UserModel:
    return UserModel(
        ...
        cv_specializations=[s.value for s in user.cv_specializations.items],
        cv_primary_languages=[l.value for l in user.cv_primary_languages.items],
...
def apply_user(model: UserModel, user: User) -> None:
    ...
    model.cv_specializations = [s.value for s in user.cv_specializations.items]
    model.cv_primary_languages = [l.value for l in user.cv_primary_languages.items]
...
def user_from_model(model: UserModel) -> User:
    ...
    work_format = (
        UserWorkFormat(model.cv_work_format) if model.cv_work_format else None
    )
    ...
    return User(
        ...
        cv_specializations=UserSpecializations.from_strs(
            model.cv_specializations or []
        ),
        cv_primary_languages=UserPrimaryLanguages.from_strs(
            model.cv_primary_languages or []
        ),
        cv_tech_stack=tech_stack,
        cv_experience_months=model.cv_experience_months,
        filter_experience_mode=(
